...
public partial class MainPage : ContentPage
{
    ...
    private void ShowSettingsTab()
    {
        ...
        _logger.LogDebug("SettingsControl.IsVisible: {IsVisible}, ResultContent.IsVisible: {ResultVisible}", 
            SettingsControl.IsVisible, ResultContent.IsVisible);
    ...
    private void OnSettingsCheckBoxChanged(object? sender, CheckBoxChangedEventArgs e)
    {
        _logger.LogInformation("OnSettingsCheckBoxChanged called for {LLMName}: {IsChecked}", e.LLMName, e.IsChecked);
        ...
        if (_tabButtons.ContainsKey(e.LLMName))
        {
            _logger.LogDebug("Found tab button for {LLMName}", e.LLMName);
            var tabButton = _tabButtons[e.LLMName];
            ...
            if (e.IsChecked)
            {
                ...
                {
                    TabHeaders.Children.Add(tabButton);
                    _logger.LogInformation("Added tab for {LLMName}", e.LLMName);
                }
                else
                {
                    _logger.LogDebug("Tab already exists for {LLMName}", e.LLMName);
                }
            }
            else
            {
                ...
                if (TabHeaders.Children.Contains(tabButton))
                {
                    ...
                    _logger.LogInformation("Removed tab for {LLMName}", e.LLMName);
                }
                else
                {
                    _logger.LogDebug("Tab not found in children for {LLMName}", e.LLMName);
                }
                
                // Remove response from dictionary
                _llmResponses.TryRemove(e.LLMName, out _);
                _logger.LogDebug("Removed response for {LLMName}", e.LLMName);
                
                // If this tab was selected, switch to Settings
                if (_currentSelectedTab == e.LLMName)
                {
                    _currentSelectedTab = "Settings";
                    SettingsTab.BackgroundColor = Colors.White;
                    ShowSettingsTab();
                    _logger.LogInformation("Switched to Settings tab because {LLMName} was deselected", e.LLMName);
                }
            }
        }
        else
        {
            _logger.LogWarning("Tab button not found for {LLMName}", e.LLMName);
    ...
    private async void OnSubmitClicked(object? sender, EventArgs e)
    {
        ...
        // Get enabled LLMs from Settings control
        var enabledLLMs = SettingsControl.GetEnabledLLMs();
        ...
        try
        {
            ...
            // Update final status
            UpdateStatusBar("All responses received", $"{_llmResponses.Count}/{enabledLLMs.Count}");
        ...
        catch (OperationCanceledException)
        {
            ...
            UpdateStatusBar("Requests cancelled", "");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error during LLM requests");
